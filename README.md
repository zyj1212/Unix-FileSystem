...
基于开源的 Unix 文件系统项目，三人协作完成以下扩展：

| 同学 | 负责模块 | 实现功能 |
|------|---------|---------|
| 同学A | 用户登录系统 | `login`、`useradd`、`whoami` |
...
- [x] 【login】
  ...
  说明：验证 /etc/passwd 中的用户信息，登录后命令提示符显示用户名
...
- [x] 【useradd】
  ...
  说明：向 /etc/passwd 追加新用户记录，uid 从 1001 开始分配
...
- [x] 【chmod】
  ...
  说明：权限值使用八进制表示，如 755、644

- [x] 【chown】

  功能：修改文件所有者（仅 root 可用）
...
- [x] 【dir】
  ...
  说明：显示文件名、物理地址、保护码、文件长度，类似 ls -l
